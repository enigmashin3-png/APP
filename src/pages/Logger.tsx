      ...
      <form onSubmit={onSubmit} className="mt-6 grid gap-3 sm:grid-cols-2">
        <label className="grid gap-1">
          ...
          <input type="date" value={date} onChange={e=>setDate(e.target.value)}
                 className="rounded-xl bg-neutral-900 border border-white/10 px-3 py-2 outline-none" required/>
        </label>
        ...
        <div className="grid grid-cols-3 gap-3 sm:col-span-2">
          ...
          <label className="grid gap-1">
            ...
            <input type="number" min={1} max={50} value={reps} onChange={e=>setReps(+e.target.value)}
                   className="rounded-xl bg-neutral-900 border border-white/10 px-3 py-2 outline-none" required/>
          </label>
          <label className="grid gap-1">
            <span className="text-sm opacity-80">Weight (kg)</span>
            <input type="number" min={0} max={1000} step="0.5" value={weight} onChange={e=>setWeight(+e.target.value)}
                   className="rounded-xl bg-neutral-900 border border-white/10 px-3 py-2 outline-none" required/>
          </label>
        </div>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-white/10">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {entries.length === 0 && (
                <tr><td className="px-3 py-3 opacity-70" colSpan={7}>No entries yet.</td></tr>
              )}
              {entries.map((e) => (
                <tr key={e.id} className="border-t border-white/5">
                  ...
                  <td className="px-3 py-2">
                    <button onClick={()=>remove(e.id)} className="underline opacity-80 hover:opacity-100">
                      Delete
                    </button>
                  </td>
                </tr>
