        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-lg font-semibold">
              Workout • {new Date(w.startedAt).toLocaleString()}
            </div>
          ...
          >
            ...
            <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 p-2 text-sm flex items-center justify-between">
              <div>
                Current best:{" "}
                {cmp.current
                  ? `${toUserUnits(cmp.current, unit).toFixed(1)} ${unitLabel(unit)}`
                  : "—"}
                {cmp.previous !== null ? (
                  <>
                    {" "}
                    • Prev: {toUserUnits(cmp.previous!, unit).toFixed(1)} {unitLabel(unit)} (
                    {cmp.delta! > 0 ? "▲" : cmp.delta! < 0 ? "▼" : "—"} {cmp.delta! > 0 ? "+" : ""}
                    {cmp.delta !== null ? toUserUnits(cmp.delta!, unit).toFixed(1) : "0"}{" "}
                    {unitLabel(unit)})
                    {cmp.previousDate && (
                      <span className="opacity-70">
                        {" "}
                        • {new Date(cmp.previousDate).toLocaleDateString()}
                      </span>
                    )}
                  </>
                ) : (
            ...
            <div className="space-y-2">
              {ex.sets.map((s, i) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between rounded-lg border px-3 h-11 border-neutral-300 dark:border-neutral-700"
                >
                  <div className="text-sm">Set {i + 1}</div>
                  <div className="text-sm opacity-80">
                    {s.weight !== undefined
                      ? `${toUserUnits(s.weight, unit)} ${unitLabel(unit)}`
                      : "—"}{" "}
                    × {s.reps ?? "—"} {s.rpe ? `• RPE ${s.rpe}` : ""}
                    {s.weight && s.reps ? (
                      <span className="ml-2 text-xs opacity-70">
                        1RM≈ {toUserUnits(est1RM(s.weight, s.reps)!, unit)} {unitLabel(unit)}
                      </span>
                    ) : null}
                  </div>
                </div>
