...
export default function SettingsPage() {
  ...
  async function doExport() {
    const blob = new Blob([JSON.stringify(await LocalActions.exportAll(), null, 2)], {
      type: "application/json",
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `liftlegends-backup-${Date.now()}.json`;
    a.click();
    ...
    <main className="p-6 max-w-3xl mx-auto space-y-8">
      ...
      <section className="rounded-2xl bg-card p-5 shadow space-y-3">
        ...
        <div className="flex gap-3">
          <button onClick={doExport} className="bg-primary text-white rounded-2xl px-4 py-2">
            Export backup
          </button>
        ...
        <p className="text-sm text-muted">
          All data is stored locally in your browser (IndexedDB). Export regularly to keep a backup.
        </p>
      </section>
    </main>
  );
